...
void bellmanFord(vector<edge>& Edges)
{
	...
	{
		updated = false;
		for(int j=0;j<E;++j)    //Relasing all edges
		{
			int U = Edges[j].src;
			int V = Edges[j].dst;
			int wt = Edges[j].wt;
...
int main()
{
	...
	for(int i=0;i<E;++i)
	{
        ...
        //for undirected graph if graph has only positive edge weights then only we can apply bellman ford, which is of no significant use as 
        //we can achieve the same using dijkstra also 
