...
//Segment-Tree (Zero-Update, Zero-Query,Kth-Zero query)
//1-based indexing
...
ll count_zero_segtree(ll curr,ll curr_l,ll curr_r,ll l,ll r,vl &segtree){
    if(l>r){
        return 0;
    }
    ...
    return count_zero_segtree(2*curr,curr_l,mid,l,min(r,mid),segtree)+count_zero_segtree(2*curr+1,mid+1,curr_r,max(l,mid+1),r,segtree);
}
...
ll kth_zero_segtree(ll curr,ll l ,ll r,ll k,vl& segtree){
    //less than k zeroes available
    if(k>segtree[curr]){
        return -1;
    }
    ...
    ll mid=(l+r)/2;
    //zero lies in the left subtree
    if(segtree[2*curr]>=k){
        return kth_zero_segtree(2*curr,l,mid,k,segtree);
    }

    //zero lies in the right subtree
    else{
        return kth_zero_segtree(2*curr+1,mid+1,r,k-segtree[2*curr],segtree);
    }
}
...
void solve(){
    ...
    //count-zero-query
    cout<<count_zero_segtree(1,0,n-1,0,2,segtree)<<endl;

    //kth-zero-query
    cout<<kth_zero_segtree(1,0,n-1,2,segtree)<<endl;

    //Update-query
    update_segtree(1,0,n-1,2,3,segtree);
    cout<<count_zero_segtree(1,0,n-1,0,2,segtree)<<endl;
