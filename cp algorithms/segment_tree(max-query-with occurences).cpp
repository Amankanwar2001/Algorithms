...
//Segment-Tree (Sum-Update, Sum-Query)
//1-based indexing
//pass the vectors as reference
//Function to compare and combine 2 pairs 
...
void build_segtree(vl &vec,ll curr,ll l,ll r,vpl &segtree,ll n){
    if(l==r){
        segtree[curr]={vec[l],1};
    }
    else{
        int mid=(l+r)/2;
...
pl max_segtree(ll curr,ll curr_l,ll curr_r,ll l,ll r,vpl &segtree){
    if(l>r){
        return {0,0};
    }
    ...
    pl seg1=max_segtree(2*curr,curr_l,mid,l,min(r,mid),segtree);
    pl seg2=max_segtree(2*curr+1,mid+1,curr_r,max(l,mid+1),r,segtree);

    return combine(seg1,seg2);

}

void update_segtree(ll curr,ll l,ll r,ll pos,ll new_val,vpl &segtree){
    if(l==r){
        segtree[curr]={new_val,1};
    }
    else{
        ll mid=(l+r)/2;
        if(pos<=mid){
...
void solve(){
    ...
    vpl segtree(4*n+1); //{max,count_of_max}pair
    //TreeBuild
    build_segtree(vec,1,0,n-1,segtree,n); //(input array,curr. vertex,start,end,segtree array,size)

    //Max-query-with-frequency
    cout<<max_segtree(1,0,n-1,0,2,segtree).ff<<" "<<max_segtree(1,0,n-1,0,2,segtree).ss<<endl;
