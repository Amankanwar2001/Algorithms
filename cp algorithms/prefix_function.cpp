...
//to calculate number of occurences of each prefix (in same as well as different string)
void each_prefix_freq(){
    string s;
    cin>>s;
    ll n=s.size();
    ...
    //for original prefixes
    for(int i=0;i<=n;i++){
        ans[i]++;
    }


    //to find occurences in a different string make sum = s+"#"+t and only take account for i>=n+1 (because we are only interested in prefixes related to string t)
}


//shortest string t such that s can be represented as a concatenation of t
void compress(){
    string s;
    cin>>s;
    ll n=s.size();
