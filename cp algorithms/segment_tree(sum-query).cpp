...
void build_segtree(vl &vec,ll curr,ll l,ll r,vl &segtree){
    if(l==r){
        segtree[curr]=vec[l];
    }
    else{
...
ll sum_segtree(ll curr,ll curr_l,ll curr_r,ll l,ll r,vl &segtree){
    if(l>r){
        return 0;
    }
    ...
    return sum_segtree(2*curr,curr_l,mid,l,min(r,mid),segtree)+sum_segtree(2*curr+1,mid+1,curr_r,max(l,mid+1),r,segtree);
}
...
void solve(){
    ...
    vl segtree(4*n+1);
    //TreeBuild
    build_segtree(vec,1,0,n-1,segtree); //(input array,curr. vertex,start,end,segtree array,size)

    //Sum-query
    cout<<sum_segtree(1,0,n-1,0,2,segtree)<<endl;
